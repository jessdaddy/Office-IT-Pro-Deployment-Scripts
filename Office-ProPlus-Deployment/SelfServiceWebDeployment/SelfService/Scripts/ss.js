...
function searchBoxFilter() {
    ...
    if (listView === 0) {
        ...
        if (searchTerm) {
            ...
            $(".custom-first-cell").each(function () {
                console.log(searchTerm + ":" + $(this).texT().toLowerCase().replace(/\ /g, "-"));
                if ($(this).text().toLocaleLowerCase().replace(" ","-").indexOf(searchTerm) >= 0) {
                    $(this).parent().parent().parent().parent().parent().parent().addClass('search-filter');
                }
            });
...
function addFilter(filter) {
    console.log(filter.replace(/\W+/g, "-").replace(/\ /g, "-"));
