    ...
    public class InstallOffice
    {
        ...
        private void ShowHelp()
        {
            Console.WriteLine("Usage: " + Process.GetCurrentProcess().ProcessName +
                              " [/uninstall] [/showxml] [/extractxml={File Path}]");
            Console.WriteLine();
        ...
        public List<string> GetEmbeddedItems(string targetDirectory)
        {
            ...
            foreach (var resourceStreamName in assembly.GetManifestResourceNames())
            {
                ...
                {
                    var fileName = Regex.Replace(resourceStreamName, "^" + assemblyName + ".", "",
        ...
        public void WaitForOfficeCtrUpadate(bool showStatus = false)
        {
            if (showStatus)
            {
                Console.WriteLine("Waiting for Install to Complete...");
            }
            ...
            do
            {
                ...
                var completeScenarios =
                    scenarioTasks.Where(s => s.State.EndsWith("ED") && !tasksDone.Contains(s.State)).ToList();
                var completeScenariosNames = completeScenarios.Select(s => s.Scenario).ToList();

                if (completeScenarios.Count > 0 && opRunning &&
                    completeScenarios.Any(s => tasksRunning.Contains(s.Scenario)))
                {
                    if (showStatus)
                    {
