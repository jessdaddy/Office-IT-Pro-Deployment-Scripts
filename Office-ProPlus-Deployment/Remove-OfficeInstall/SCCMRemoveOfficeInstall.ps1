function Retrieve-OfficeResetFiles {
<#
.SYNOPSIS
This method will retrieve all the Reset files to the SCCM shared folder specified
.DESCRIPTION
This method is used to copy files used for Office reset from their current directory to a directory where clients have access to them
.PARAMETER Path
The path to the UNC share to copy the reset files to
.Example
Retrieve-OfficeResetFiles
Default without parameters specified this will create a local folder named 'OfficeUpdates' on the system drive and then create a hidden share named 'OfficeUpdates$'. It will then copy the reset files to that folder.
.Example
Download-OfficeUpdates -Path "\\Server\OfficeShare"
If you do not want to host the reset files on the local server you can specify a UNC share path. The script must be run with a user account that has Read/Write permissions to the share. 
#>
    ...
    Process
    {
        ...
        if (!(Test-Path -Path "$Path\SCCMRemoveInstall.exe")) {
              Copy-Item -Path ".\*" -Destination $Path
        }
...
function Setup-OfficeResetPackage {
<#
.SYNOPSIS
Automates the configuration of System Center Configuration Manager (SCCM) to configure a complete office uninstall and reinstall
.DESCRIPTION
.PARAMETER path
The UNC Path where the reset files will be stored for installation to the target machines.
.PARAMETER $SiteCode
...
.Example
Setup-SCCMOfficeUpdates -Path \\SCCM-CM\OfficeReset -PackageName "Remove Office Install" -ProgramName "LaunchOfficeReInstall.exe" -distributionPoint SCCM-CM.CONTOSO.COM
#>
...
(
	...
	[Parameter()]
	[String]$PackageName = "Remove Office Install",
		
	[Parameter()]
	[String]$ProgramName = "LaunchOfficeReInstall.exe",
...
function Deploy-OfficeResetPackage {
<#
.SYNOPSIS
Automates the configuration of System Center Configuration Manager (SCCM) to deploy office reset scripts
.DESCRIPTION
.PARAMETER Collection
The target SCCM Collection
.PARAMETER PackageName
The Name of the SCCM package created by the Setup-OfficeResetPackage function
.PARAMETER ProgramName
The Name of the SCCM program created by the Setup-OfficeResetPackage function
.PARAMETER UpdateOnlyChangedBits
Determines whether or not the EnableBinaryDeltaReplication enabled or not
.PARAMETER SCCMPSModulePath
Allows the user to specify that full path to the ConfigurationManager.psd1 PowerShell Module. This is especially useful if SCCM is installed in a non standard path.
.Example
Deploy-OfficeResetPackage -Collection "CollectionName"
Deploys the Package created by the Setup-OfficeResetPackage function
#>
    [CmdletBinding()]	
    Param
	(	
...
function CreateSCCMPackage() {
	...
	(
		[Parameter()]
		[String]$Name = "Remove Office Install",
...
function CreateSCCMProgram() {
	...
	(
		...
		[Parameter()]
		[String]$Name = "LaunchOfficeReInstall.exe",
    ...
    $commandLine = "LaunchOfficeReInstall.exe"
    ...
    if($program -eq $null -or !$program)
    {
	    ...
	    $program = New-CMProgram -PackageName $PackageName -StandardProgramName $Name -DriveMode RenameWithUnc -CommandLine $commandLine -ProgramRunType WhetherOrNotUserIsLoggedOn -RunMode RunWithAdministrativeRights -UserInteraction $false -RunType Maximized
    } else {
        Write-Host "`t`tAlready Exists"
    }
